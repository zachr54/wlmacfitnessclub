...
begin
  -- auth.uid() is null when the change comes from the Supabase dashboard/SQL editor
  -- (not through the app) — always allow those; only guard app-session changes.
  if new.role <> old.role and auth.uid() is not null and not public.is_exec() then
    new.role := old.role;
  end if;
  return new;
end;
...
create policy "avatar_upload_own" on storage.objects for insert with check (bucket_id = 'avatars' and auth.uid()::text = (storage.foldername(name))[1]);
create policy "avatar_update_own" on storage.objects for update using (bucket_id = 'avatars' and auth.uid()::text = (storage.foldername(name))[1]);
